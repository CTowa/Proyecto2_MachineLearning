...
# Small safety setting: avoids some machines getting slow or stuck with too many math threads.
...
def configure_plots() -> None:
    # This is mostly for the PDF: the teacher can penalize plots with tiny labels.
    # Keeping the sizes high here makes the exported PNGs safer when LaTeX scales them.
    plt.rcParams.update(
        {
            "figure.figsize": (12, 8),
            "figure.dpi": 140,
...
def save_dataset_summary(
    ...
    plt.tight_layout()
    plt.savefig(paths.figures / "target_distribution.png", dpi=240)
    plt.close()
...
def run_clustering_analysis(x_scaled: np.ndarray, paths: OutputPaths) -> pd.DataFrame:
    ...
    # Quick diagnostic plot: it helps explain why DBSCAN eps was not picked blindly.
    plt.figure(figsize=(12, 7))
    plt.plot(np.sort(kth_neighbor_distances))
    plt.title("Curva k-distancia para selección de eps en DBSCAN")
    plt.xlabel("Observaciones ordenadas")
    plt.ylabel(f"Distancia al vecino {min_samples}")
    plt.tight_layout()
    plt.savefig(paths.figures / "dbscan_k_distance.png", dpi=240)
    plt.close()
    ...
    # Another small diagnostic plot: it supports the GMM component selection discussion.
    gmm_metrics = metrics_df[metrics_df["algorithm"] == "GMM"].copy()
    if not gmm_metrics.empty and "bic" in gmm_metrics.columns:
        gmm_metrics["n_components"] = (
            gmm_metrics["hyperparameters"]
            .str.extract(r"n_components=(\d+)")[0]
            .astype(int)
        )

        plt.figure(figsize=(12, 7))
        plt.plot(gmm_metrics["n_components"], gmm_metrics["bic"], marker="o")
        plt.title("BIC por número de componentes en GMM")
        plt.xlabel("Número de componentes")
        plt.ylabel("BIC")
        plt.tight_layout()
        plt.savefig(paths.figures / "gmm_bic_curve.png", dpi=240)
        plt.close()
...
def plot_mlp_histories(histories: dict[str, pd.DataFrame], paths: OutputPaths) -> None:
    # Loss curves with train and validation are easier to defend in the report.
    plt.figure(figsize=(13, 8))
    for variant, history in histories.items():
        plt.plot(history["epoch"], history["train_loss"], label=f"{variant} - train")
        plt.plot(history["epoch"], history["valid_loss"], linestyle="--", label=f"{variant} - valid")
    plt.title("Curvas de pérdida de entrenamiento y validación")
    plt.xlabel("Época")
    plt.ylabel("Loss")
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig(paths.figures / "mlp_loss_curves.png", dpi=240)
    plt.close()

    plt.figure(figsize=(13, 8))
    for variant, history in histories.items():
        plt.plot(history["epoch"], history["valid_f1_macro"], label=variant)
    plt.title("F1 macro de validación en MLP")
    plt.xlabel("Época")
    plt.ylabel("F1 macro")
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig(paths.figures / "mlp_f1_curves.png", dpi=240)
    plt.close()
...
def save_confusion_matrix(
    ...
    # Absolute matrix: keeps the current report figure intact.
    ...
    # Normalized matrix: useful for class imbalance because each row sums to one.
    normalized_path = output_path.with_name(output_path.stem + "_normalized.png")
    fig, ax = plt.subplots(figsize=(10, 9))
    ConfusionMatrixDisplay.from_predictions(
        y_true,
        y_pred,
    ...
    ax.set_title(title + " normalizada")
    plt.tight_layout()
    plt.savefig(normalized_path, dpi=240)
    plt.close(fig)
